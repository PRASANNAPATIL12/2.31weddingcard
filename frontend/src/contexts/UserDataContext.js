...
export const UserDataProvider = ({ children }) => {
  ...
  useEffect(() => {
    const checkAuth = () => {
      ...
      if (sessionId && userId && username) {
        ...
        // Load user's wedding data from localStorage
        const savedData = localStorage.getItem(`wedding_data_${userId}`);
        if (savedData) {
          try {
  ...
  // Save wedding data to localStorage and backend whenever it changes (for authenticated users)
  const saveWeddingData = async (newData) => {
    setWeddingData(newData);
    
    if (isAuthenticated && userInfo?.userId) {
      // Save to localStorage with user-specific key
      localStorage.setItem(`wedding_data_${userInfo.userId}`, JSON.stringify(newData));
      
      // Also save to backend for public URL access
      try {
        const backendUrl = process.env.REACT_APP_BACKEND_URL || 'http://localhost:8001';
        const dataToSend = { ...newData, session_id: userInfo.sessionId };
        ...
        if (!response.ok) {
          // If update fails, try creating new wedding data
          const createResponse = await fetch(`${backendUrl}/api/wedding`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
          ...
          if (!createResponse.ok) {
            console.error('Failed to save wedding data to backend');
          }
        }
      } catch (error) {
        console.error('Error saving wedding data to backend:', error);
        // Continue with localStorage only if backend fails
      }
    }
  };
