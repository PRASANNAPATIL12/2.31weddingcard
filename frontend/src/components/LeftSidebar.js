...
const LeftSidebar = () => {
  ...
  const sidebarSections = [
    ...
    { id: 'url', label: 'Get Custom URL', icon: Link, color: '#3B82F6' },
  ...
  const handlePremiumFeature = (featureId) => {
    // Use custom URL if available, otherwise fallback to user ID
    ...
    switch (featureId) {
      ...
      case 'qrcode':
        ...
        const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(weddingUrl)}`;
