...
const DashboardPage = () => {
  ...
  const loadWeddingData = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setWeddingData(data);
        generateWeddingUrl(data.id);
      }
    } catch (error) {
      console.log('No existing wedding data found, using defaults');
    }
  };

  const generateWeddingUrl = (weddingId) => {
    if (weddingId) {
      setWeddingUrl(`${window.location.origin}/wedding/${weddingId}`);
    }
  };
  ...
  const sidebarSections = [
    ...
    { id: 'url', label: 'Get My URL', icon: LinkIcon },
